...
const SPAM_KEYWORDS = ["free", "click here", "limited time", "gift card", "winner", "subscribe", "buy now", "discount", "offer", "promo"];
const TOXIC_KEYWORDS = ["worthless", "garbage", "terrible", "hate", "stupid", "idiot", "worst", "trash", "pathetic", "disgusting", "moron", "dumb", "useless", "awful", "horrible", "suck"];
const POSITIVE_KEYWORDS = ["great", "love", "awesome", "amazing", "excellent", "good", "thank", "wonderful", "fantastic", "helpful", "enjoy", "nice", "beautiful", "perfect", "brilliant", "impressive", "pleased", "happy"];

export function analyzeText(input: string): ModerationDemoResult {
  const lower = input.toLowerCase();
  const words = lower.split(/\s+/);

  // Spam detection
  const capsRatio = input.replace(/[^a-zA-Z]/g, "").length > 0
    ? (input.replace(/[^A-Z]/g, "").length / input.replace(/[^a-zA-Z]/g, "").length)
    : 0;
  const hasExcessivePunctuation = /[!?]{3,}/.test(input);
  const hasUrl = /https?:|www\.|\.com|\.net|\.org/i.test(input);
  const spamCount = SPAM_KEYWORDS.filter(kw => lower.includes(kw)).length;
  const isSpam = spamCount >= 2 || (capsRatio > 0.5 && words.length > 2) || (hasExcessivePunctuation && hasUrl);
  ...
  if (isSpam) {
    return {
      ...
      rules: {
        checks: [
          { name: "Spam pattern check", passed: spamCount === 0 },
          { name: "Blocklist filter", passed: true },
          { name: "Intensity check", passed: !hasExcessivePunctuation },
          { name: "URL pattern check", passed: !hasUrl },
        ],
        flagged: true,
        flagReason: [
          spamCount > 0 && "Spam patterns",
          capsRatio > 0.5 && "Excessive capitalization",
          hasExcessivePunctuation && "Excessive punctuation",
        ].filter(Boolean).join(", "),
      },
      toxicity: { toxic: 0.05, severe_toxic: 0.0, obscene: 0.02, threat: 0.0, insult: 0.01, identity_hate: 0.0 },
  ...
  // Toxicity detection
  const toxicMatches = TOXIC_KEYWORDS.filter(kw => lower.includes(kw));
  const toxicCount = toxicMatches.length;

  if (toxicCount >= 3) {
    const intensity = Math.min(0.95, 0.6 + toxicCount * 0.1);
    return {
      label: "Custom",
      input,
      rules: {
      ...
      toxicity: { toxic: intensity, severe_toxic: intensity * 0.4, obscene: intensity * 0.5, threat: 0.08, insult: intensity, identity_hate: intensity * 0.15 },
      sentiment: { score: 0.08, label: "Very Negative" },
      decision: "REJECT",
      confidence: 0.9,
    };
  }
  ...
  if (toxicCount >= 1) {
    const intensity = 0.25 + toxicCount * 0.15;
    return {
      label: "Custom",
      input,
      rules: {
      ...
      toxicity: { toxic: intensity, severe_toxic: 0.02, obscene: intensity * 0.6, threat: 0.01, insult: intensity + 0.1, identity_hate: 0.01 },
  ...
  if (positiveCount >= 1) {
    return {
      ...
      toxicity: { toxic: 0.02, severe_toxic: 0.0, obscene: 0.01, threat: 0.0, insult: 0.02, identity_hate: 0.0 },
  ...
  return {
    ...
    toxicity: { toxic: 0.04, severe_toxic: 0.0, obscene: 0.02, threat: 0.0, insult: 0.03, identity_hate: 0.0 },
