        ...
        <AnimatePresence mode="wait">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {doc.chunks.map((chunk, ci) => (
                        <motion.div
                          ...
                          className="bg-gray-900 rounded px-3 py-2 text-xs text-gray-300 border border-gray-800 max-w-[250px]"
                        >
                          <span className="text-sky-300 text-[11px] mr-1">
                            [{di}.{ci}]
                          </span>
                          {chunk.slice(0, 80)}...
                        </motion.div>
                ...
                {RAG_SAMPLE_DOCUMENTS[0].chunks.map((chunk, i) => (
                  ...
                  >
                    ...
                    <motion.div
                      ...
                      className="text-[11px] font-mono text-sky-300 bg-sky-300/10 px-2 py-1 rounded"
                    >
          ...
          {stage === "searching" && (
            ...
            >
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                className="bg-sky-300/10 border border-sky-300/30 rounded-lg p-3 text-center"
              >
                <p className="text-xs text-gray-300 mb-1">Query</p>
                ...
                (match, i) => {
                  ...
                  return (
                    <motion.div
                      ...
                      animate={{ opacity: 1, y: 0 }}
                      transition={{ delay: i * 0.2 }}
                      className="flex items-center gap-3 bg-gray-900 rounded-lg p-3 border border-sky-300/30"
                    >
                      <span className="text-sky-300 font-bold text-sm w-6">
                        #{i + 1}
                      </span>
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-gray-300">{chunk}</p>
                      </div>
                      <span className="text-xs font-mono text-sky-300 bg-sky-300/10 px-2 py-1 rounded">
                        {match.score.toFixed(2)}
                      </span>
                    </motion.div>
                  );
          ...
          {stage === "response" && (
            <motion.div
              ...
              className="space-y-4"
            >
              <div className="bg-sky-300/5 border border-sky-300/20 rounded-lg p-4">
                <p className="text-xs text-sky-300 mb-2 font-medium">
                  Generated Response
                </p>
                <motion.p
                  initial={{ opacity: 0 }}
                ...
                <div className="flex flex-wrap gap-2">
                  {RAG_SAMPLE_RESPONSE.sources.map((src, i) => (
                    <motion.span
                      ...
                      className="text-xs bg-gray-900 text-sky-300 px-2 py-1 rounded border border-sky-300/20"
                    >
                      [{i + 1}] {src}
                    </motion.span>
                  ))}
