...
import { MODERATION_EXAMPLES, analyzeText, type ModerationDemoResult } from "@/lib/demoData";
...
function ToxicityBar({
  ...
  return (
    <div className="flex items-center gap-2">
      <span className="text-[11px] text-gray-300 w-20 text-right">{label}</span>
      ...
      <span className="text-[10px] font-mono text-gray-300 w-8">
        {value.toFixed(2)}
      </span>
    </div>
  );
    ...
    <div className="flex flex-col items-center gap-2">
      <div className="relative w-32 h-16 overflow-hidden">
        ...
        <motion.div
          ...
          className="absolute bottom-0 left-1/2 origin-bottom w-0.5 h-14 bg-sky-300"
          style={{ transformOrigin: "bottom center", marginLeft: "-1px" }}
        />
        <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-3 h-3 rounded-full bg-sky-300" />
      </div>
      <div className="flex justify-between w-32 text-[10px] text-gray-400">
        <span>Negative</span>
        <span>Neutral</span>
        <span>Positive</span>
      </div>
...
export default function ModerationDemo() {
  ...
  const [selectedExample, setSelectedExample] = useState<ModerationDemoResult | null>(null);
      ...
      <div className="space-y-3">
        ...
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 px-4 py-2.5 rounded-lg bg-gray-50 border border-gray-200 text-sm text-foreground placeholder-gray-500 focus:outline-none focus:border-accent-blue transition-colors"
            disabled={stage !== "idle"}
          />
          <button
            onClick={() => startModeration(selectedExample || analyzeText(input))}
      ...
      <div className="bg-gray-950 rounded-xl p-6 min-h-[280px] text-gray-200">
        <AnimatePresence mode="wait">
          ...
          {stage === "ensemble" && result && (
            ...
            >
              <p className="text-xs text-sky-300 font-medium self-start">
                Ensemble Decision
              </p>
              <div className="flex items-center gap-4">
                {["Rules", "Toxicity", "Sentiment"].map((layer, i) => (
                  <motion.div
                    ...
                    className="text-center"
                  >
                    <div className="bg-gray-900 rounded-lg px-3 py-2 border border-gray-800 text-xs text-gray-300">
                      {layer}
                    </div>
                  </motion.div>
                ))}
        ...
        {stage !== "idle" && (
          ...
          >
            {isLastStage ? (
              <button
                onClick={reset}
                className="flex items-center gap-2 px-4 py-2 rounded-lg bg-gray-900 hover:bg-gray-800 text-gray-300 hover:text-white text-sm transition-colors border border-gray-800"
              >
                <RotateCcw size={14} />
                Try Another
              </button>
